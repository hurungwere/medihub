...
const sidebarItems = [
  ...
  { icon: '⭐', label: 'Watchlist', href: '/dashboard/supplier/watchlist', id: 'watchlist' },
  { icon: '⚙️', label: 'Profile', href: '/dashboard/supplier/profile', id: 'profile' },
]

export default function SupplierLayout({ children }: { children: React.ReactNode }) {
  const [collapsed, setCollapsed] = useState(false)
  const [userName, setUserName] = useState('Supplier User')
  const [userOrg, setUserOrg] = useState('Supplier Account')
  ...
  useEffect(() => {
    ...
    if (cookiesObj['session_user']) {
      try {
        ...
        setUserName(user.name || user.email || 'Supplier User')
        setUserOrg(user.organization || 'Supplier Account')
      } catch (e) {}
    }
  }, [])
      ...
      <aside className={`hidden lg:flex flex-col bg-slate-900 border-r border-slate-800/60 transition-all duration-300 ${collapsed ? 'w-16' : 'w-60'}`}>
        <div className={`flex items-center gap-2.5 p-4 border-b border-slate-800/60 h-20 ${collapsed ? 'justify-center' : ''}`}>
          {collapsed ? (
            ...
            <div className="flex flex-col mt-2">
              ...
              <p className="text-[10px] text-emerald-400 font-semibold leading-none">Supplier Portal</p>
            </div>
          )}
        </div>
        {!collapsed && (
          <div className="mx-3 my-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
            <p className="text-xs font-semibold text-emerald-400">Supplier Portal</p>
            <p className="text-xs text-slate-500 truncate">{userOrg}</p>
          </div>
        )}
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto no-scrollbar">
          {sidebarItems.map(item => (
            <Link key={item.id} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-slate-400 hover:text-white hover:bg-slate-800 ${collapsed ? 'justify-center' : ''}`}>
        ...
        <div className={`p-3 border-t border-slate-800/60 flex items-center justify-between gap-2 ${collapsed ? 'justify-center' : ''}`}>
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center text-xs font-bold text-white flex-shrink-0">
            ...
            {!collapsed && (
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[10px] text-slate-500 truncate">Verified Supplier</p>
                </div>
              </div>
            )}
          </div>
        ...
        <button onClick={() => setCollapsed(!collapsed)} className="p-3 border-t border-slate-800/60 text-slate-500 hover:text-slate-300 transition-colors flex justify-center">
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        <header className="h-16 bg-slate-900/80 backdrop-blur-xl border-b border-slate-800/60 flex items-center justify-between px-4 lg:px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            <h1 className="text-sm font-semibold text-slate-300">Supplier Portal</h1>
            <span className="hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-400 font-medium">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="w-9 h-9 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-colors relative">
              ...
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 text-white text-xs rounded-full flex items-center justify-center">5</span>
            </button>
          </div>
        </header>
